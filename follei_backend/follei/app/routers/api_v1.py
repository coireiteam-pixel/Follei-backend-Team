...
def queue_background_job(payload: BackgroundJobRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    job_id = uuid4()
    db.execute(text("INSERT INTO background_jobs (id, job_type, status, payload, scheduled_at, created_at, updated_at) VALUES (:id, :job_type, 'queued', :payload, :scheduled_at, :now, :now)"), {"id": job_id, "job_type": payload.type, "payload": _pg_json({"priority": payload.priority, **payload.payload}), "scheduled_at": payload.scheduled_at, "now": _now()})
    db.commit()
...
def send_notification(payload: NotificationRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    db.execute(text("INSERT INTO notifications (id, tenant_id, user_id, notification_type, title, body, payload, created_at) VALUES (:id, :tenant_id, :user_id, :type, :title, :body, :payload, :now)"), {"id": notification_id, "tenant_id": tenant_id, "user_id": payload.user_id, "type": payload.type, "title": payload.title, "body": payload.body, "payload": _pg_json({"priority": payload.priority, **payload.data}), "now": _now()})
    db.commit()
...
def create_feature_flag(payload: FeatureFlagRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    flag_id = uuid4()
    db.execute(text("INSERT INTO feature_flags (id, flag_key, is_enabled, config, created_at, updated_at) VALUES (:id, :key, :enabled, :config, :now, :now)"), {"id": flag_id, "key": payload.name, "enabled": payload.enabled, "config": _pg_json({"description": payload.description, "target": payload.target}), "now": _now()})
    db.commit()
