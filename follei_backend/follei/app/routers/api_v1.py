...
def _token_pair(user_id: str, tenant_id: str) -> dict[str, Any]:
...
def _roles_for_user(db: Session, user_id: str, fallback: str | None = None) -> list[str]:
    rows = db.execute(
        text(
            """
            SELECT r.name
...
def refresh(payload: RefreshRequest) -> dict[str, Any]:
    ...
    return {
        "access_token": create_access_token(token_payload["sub"], str(token_payload["tenant_id"])),
        "expires_in": TOKEN_EXPIRES_IN,
    }
...
def create_tenant(payload: TenantCreateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    db.execute(
        ...
        {"id": _db_uuid(short_id()), "tenant_id": _db_uuid(tenant_id), "settings": _db_json(payload.settings), "now": _now()},
    )
    db.commit()
...
def get_tenant(tenant_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def update_tenant(tenant_id: str, payload: TenantUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def get_tenant_settings(tenant_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def update_tenant_settings(tenant_id: str, payload: TenantSettingsUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    db.execute(
        ...
        {"id": _db_uuid(short_id()), "tenant_id": _db_uuid(tenant_id), "settings": _db_json(settings), "now": _now()},
    )
    db.commit()
...
def tenant_usage(tenant_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
    return {
...
def list_users(tenant_id: str | None = None, role: str | None = None, status: str | None = None, page: int = 1, page_size: int = 20, db: Session = Depends(get_db)) -> dict[str, Any]:
    filters = []
...
def update_user(user_id: str, payload: UserUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    if payload.full_name:
        first_name, last_name = _full_name_parts(payload.full_name)
...
def deactivate_user(user_id: str, db: Session = Depends(get_db)) -> Response:
...
def assign_role(user_id: str, payload: AssignRoleRequest, db: Session = Depends(get_db)) -> dict[str, str]:
...
def remove_role(user_id: str, role_id: str, db: Session = Depends(get_db)) -> Response:
...
def create_api_key(payload: ApiKeyCreateRequest, current_user: dict[str, Any] = Depends(_current_user), db: Session = Depends(get_db)) -> dict[str, Any]:
    key = f"fl_live_{short_id()}{short_id()}"
    key_id = short_id()
...
def revoke_api_key(key_id: str, db: Session = Depends(get_db)) -> Response:
...
def create_agent(payload: AgentCreateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    db.execute(text("INSERT INTO agent_versions (id, tenant_id, agent_id, version, model, system_prompt, config, created_at) VALUES (:id, :tenant_id, :agent_id, 1, :model, :system_prompt, :config, :now)"), {"id": _db_uuid(short_id()), "tenant_id": _db_uuid(payload.tenant_id), "agent_id": _db_uuid(agent_id), "model": payload.config.get("model"), "system_prompt": system_prompt, "config": _db_json(payload.config), "now": _now()})
    db.commit()
...
def list_agents(tenant_id: str | None = None, type: str | None = None, status: str | None = None, page: int = 1, page_size: int = 20, db: Session = Depends(get_db)) -> dict[str, Any]:
    filters = []
    params: dict[str, Any] = {"limit": page_size, "offset": (page - 1) * page_size}
    if tenant_id:
        filters.append("tenant_id = :tenant_id")
...
def update_agent(agent_id: str, payload: AgentUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]: # This function needs further updates for config/status
...
def chat(agent_id: str, payload: AgentChatRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    conversation_id = payload.conversation_id or short_id()
    message_id = short_id()
...
def snapshot_agent(agent_id: str, payload: AgentVersionRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    version = db.execute(text("SELECT COALESCE(max(version), 0) + 1 FROM agent_versions WHERE agent_id = :id"), {"id": _db_uuid(agent_id)}).scalar_one()
    
    config_json = _db_json({"name": payload.name, "notes": payload.notes})
    
    db.execute(text("INSERT INTO agent_versions (id, tenant_id, agent_id, version, system_prompt, config, created_at) VALUES (:id, :tenant_id, :agent_id, :version, :system_prompt, :config, :now)"), {"id": _db_uuid(short_id()), "tenant_id": _db_uuid(agent["tenant_id"]), "agent_id": _db_uuid(agent_id), "version": version, "system_prompt": agent["system_prompt"], "config": config_json, "now": _now()})
    db.commit()
...
def list_agent_versions(agent_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def start_agent_session(agent_id: str, payload: AgentSessionRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    session_id = short_id()
...
def list_agent_sessions(agent_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def update_agent_session(agent_id: str, session_id: str, payload: AgentSessionUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def create_agent_task(agent_id: str, payload: AgentTaskRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    task_id = short_id()
...
def list_agent_tasks(agent_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def update_agent_task(task_id: str, payload: AgentTaskUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def store_agent_memory(agent_id: str, payload: AgentMemoryRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    memory_id = short_id()
...
def list_agent_memories(agent_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def submit_agent_feedback(agent_id: str, payload: AgentFeedbackRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    feedback_id = short_id()
...
def list_agent_feedback(agent_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def grant_tool_permission(agent_id: str, payload: ToolPermissionRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    agent = get_agent(agent_id, db)
    permission_id = short_id()
...
def list_tool_permissions(agent_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def audit_logs(tenant_id: str | None = None, user_id: str | None = None, action: str | None = None, resource: str | None = None, from_: datetime | None = Query(None, alias="from"), to: datetime | None = None, db: Session = Depends(get_db)) -> dict[str, Any]:
    filters = []
    params: dict[str, Any] = {}
    if tenant_id:
        filters.append("tenant_id = :tenant_id")
...
def get_background_job(job_id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def mark_notification_read(id: str, db: Session = Depends(get_db)) -> dict[str, Any]:
    now = _now()
...
def toggle_feature_flag(flag_id: str, payload: FeatureFlagUpdateRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
...
def api_request_logs(tenant_id: str | None = None, endpoint: str | None = None, status_code: int | None = Query(None, alias="status"), from_: datetime | None = Query(None, alias="from"), db: Session = Depends(get_db)) -> dict[str, Any]:
    filters = []
    params: dict[str, Any] = {}
    if tenant_id:
        filters.append("tenant_id = :tenant_id")
